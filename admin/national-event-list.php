...
<div class="container-xxl flex-grow-1" style="padding: 0px 10px;">
    <div class="card mb-2">
        <div class="row p-2">
            <div class="col-md-8">
                <h6 class="card-header"><i class="ri-building-line"></i> National Event Details</h6>
            </div>
            <?php if(get_permission('national_event', 'is_add')){ ?>
    ...
    <div class="card">
        <div class="table-responsive text-nowrap">
            <table id="clubTable" class="table" style="width: 100%;"> 
                <thead>
                    <tr>  <?php if(get_permission('national_event', 'is_edit') ||  get_permission('national_event', 'is_delete')){ ?>
                        <th>Actions</th>
                        <?php } ?>
...
$('.add_btn').on('click', function() {
    window.location.href = 'state-events.php';
});
...
function getDataTable() {    
    ...
    destroy: true,
    ajax: {
        url: `api/n-events/read.php${queryString}`,
        type: 'GET'
    },
    ...
    columns: [
        {
            ...
            render: (data) => {
                ...
                // === EXISTING BUTTONS ===
                html += `<?php if(get_permission('national_event', 'is_edit')){ ?>
                    <a class="btn btn-info btn-xs" href="events-result.php?event_id=${data.id}">
                    ...
                    <a class="btn btn-success btn-xs" href="events-report-state.php?event_id=${data.id}">
                        <i class="ri-edit-box-fill"></i> View Event Register List
                    </a>

                    <a class="btn btn-secondary btn-xs" target="_blank"
                       href="certificates/certificate/s-certificate.php?event_id=${data.id}&limit=0,1000&districtId=${districtId}">
                        All Certificate Print
                    </a>
                <?php } ?>`;

                // === DELETE PERMISSION (EMPTY FOR NOW) ===
                html += `<?php if(get_permission('national_event', 'is_delete')){ ?>
                <?php } ?>`;

                // === Print Certificate Batch Dropdown (ONLY USER_ID = 1) ===
                    ...
                    for (let i = 0; i < total; i += batch) {
                        ...
                        dropdown += `<li>
                            <a class="dropdown-item" target="_blank"
                               href="certificates/certificate/s-certificate.php?event_id=${data.id}&districtId=${districtId}&limit=${i},${batch}">
                                ${start} - ${end}
                            </a>
                        </li>`;
                    }
        ...
        <?php if($_SESSION['role_id']==2 || $_SESSION['role_id']==4){ ?>
        {
            ...
            render: (data) => `
                ...
                <a class="btn btn-secondary btn-xs"
                   href="excel/national_selection_list.php?event_id=${data.id}">
                    National Selection List
                </a>
            `
        },
        <?php } ?>
...

        {
            ...
            render: (data) => {
                ...
                const formattedTime = date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }).replace(':', '.');
...
function editClub(id) {
   window.location.href = 'national-events.php?id='+id;
}
