...
$requiredFields = ['event_name', 'state_id', 'district_id', 'event_date', 'reg_start_date', 'reg_end_date', 'event_fees', 'venue', 'association_name', 'reg_no', 'title_of_championship', 'date','date1'];
...
$postData['event_level_type_id'] =  1;
...
function uploadFile($fileKey, $uploadDir, $allowedExtensions) {
    ...
    return 'uploads/d-events/' . $newFileName; // Store relative path in DB
}
...
try {
    $stmt = $pdo->prepare("INSERT INTO tbl_events 
        (event_name, event_level_type_id, state_id, district_id, event_date, reg_start_date, reg_end_date, event_fees,event_remarks, venue, title_of_championship, date,date1, association_name, reg_no, event_description, status, event_image, secretory_sign, president_sign, created_at, updated_at) 
        VALUES (:event_name, :event_level_type_id, :state_id, :district_id, :event_date, :reg_start_date, :reg_end_date, :event_fees,:event_remarks, :venue, :title_of_championship, :date,:date1, :association_name, :reg_no, :event_description, :status, :event_image, :secretory_sign, :president_sign, NOW(), NOW())");
    ...
    if ($stmt->execute()) {
        echo json_encode(["status" => "success", "message" => "District Event created successfully"]);
    } else {
