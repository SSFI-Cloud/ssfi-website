...
$columns = ['id', 'name', 'address', 'city', 'state', 'pincode', 'mobile_number', 'status', 'branch_id', 'created_by', 'updated_by', 'created_at', 'updated_at'];
    ...
    // Fetch data query
    $sqlData = "SELECT * FROM tbl_pickup_center  $whereClause ORDER BY $orderBy $orderDir";
    ...
    $sqlTotal = "SELECT COUNT(*) as total FROM tbl_pickup_center";
    $totalRecords = $pdo->query($sqlTotal)->fetchColumn();

    // Total filtered records count (with filtering)
    $sqlFiltered = "SELECT COUNT(*) as total FROM tbl_pickup_center $whereClause";
