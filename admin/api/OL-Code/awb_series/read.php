...
try {
    ...
    $columns = ['id', 'branch_id', 'product_id', 'consignment_type_id', 'start', 'end', 'created_at', 'updated_at', 'created_by', 'updated_by', 'status'];
    ...
    // Fetch data query
    $sqlData = "SELECT c.name AS consignment_name,p.name as product_name,b.branch_name, a.*
                FROM awb_series_allocation a
                LEFT JOIN tbl_consignement_type c ON a.consignment_type_id = c.id
                LEFT JOIN tbl_products p ON a.product_id = p.id
                LEFT JOIN tbl_branch b ON a.branch_id = b.id  $whereClause ORDER BY $orderBy $orderDir";
    ...
    $sqlTotal = "SELECT COUNT(*) as total FROM awb_series_allocation";
    $totalRecords = $pdo->query($sqlTotal)->fetchColumn();

    // Total filtered records count (with filtering)
    $sqlFiltered = "SELECT COUNT(*) as total FROM awb_series_allocation $whereClause";
