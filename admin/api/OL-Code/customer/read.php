...
try {
    ...
    $columns = ['id', 'name','mobile','address1','address2','pincode', 'status', 'created_by', 'created_at'];
    ...
    // Fetch data query
    $sqlData = "SELECT * FROM tbl_customer  $whereClause ORDER BY $orderBy $orderDir";
    ...
    $sqlTotal = "SELECT COUNT(*) as total FROM tbl_customer";
    $totalRecords = $pdo->query($sqlTotal)->fetchColumn();

    // Total filtered records count (with filtering)
    $sqlFiltered = "SELECT COUNT(*) as total FROM tbl_customer $whereClause";
