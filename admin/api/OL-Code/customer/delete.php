...
// Prepare and execute the query to delete customer data
try {
    
    
    // Check if the user exists in the database
        $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM tbl_customer WHERE id = :id");
        $checkStmt->execute([':id' => $id]);
        $userExists = $checkStmt->fetchColumn();

        if ($userExists > 0) {
            $stmt = $pdo->prepare("DELETE FROM tbl_customer WHERE id = :id");
            $stmt->execute([':id' => $id]);
            echo json_encode(["success" => true, "message" => "Customer deleted successfully"]);
        } else {
            echo json_encode(["success" => false, 'message' => 'No customer found or already deleted']);
        }
} catch (PDOException $e) {
    error_log("Error deleting customer: " . $e->getMessage());
    echo json_encode(["success" => false, 'error' => 'Error deleting customer: ' . $e->getMessage()]);
}
?>
