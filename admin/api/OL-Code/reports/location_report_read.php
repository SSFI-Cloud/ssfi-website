...
try {
    ...
    // Fetch data query
    $sqlData = "SELECT * FROM tbl_city  $whereClause ORDER BY $orderBy $orderDir";
    ...
    $sqlTotal = "SELECT COUNT(*) as total FROM tbl_city";
    $totalRecords = $pdo->query($sqlTotal)->fetchColumn();

    // Total filtered records count (with filtering)
    $sqlFiltered = "SELECT COUNT(*) as total FROM tbl_city $whereClause";
