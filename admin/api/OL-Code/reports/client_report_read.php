...
try {
    ...
    // Fetch data query
    $sqlData = "SELECT b.branch_name,c.*
                FROM tbl_client c
                LEFT JOIN tbl_branch b ON c.branch_id = b.id  $whereClause ORDER BY $orderBy $orderDir";
    ...
    $sqlTotal = "SELECT COUNT(*) as total FROM tbl_client";
    $totalRecords = $pdo->query($sqlTotal)->fetchColumn();

    // Total filtered records count (with filtering)
    $sqlFiltered = "SELECT COUNT(*) as total FROM tbl_client $whereClause";
