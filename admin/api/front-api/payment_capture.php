...
if (isset($data['razorpay_payment_id']) && isset($data['razorpay_order_id'])) {
    ...
    if ($http_status === 200 && isset($payment_response['status'])) {
        if ($payment_response['status'] == "captured") {
            ...
            // $stmt = $pdo->query("SELECT skater_id FROM tbl_session_renewal where payment_id=?");
            // $stmt->execute([$data['razorpay_payment_id']]);
            // $skater_id = $stmt->fetchColumn();     
                // Prepare the SQL statement
$stmt = $pdo->prepare("SELECT skater_id FROM tbl_session_renewal WHERE payment_id = :payment_id");
...
$stmt = $pdo->prepare("SELECT age_category FROM tbl_session_renewal WHERE payment_id = :payment_id");
$stmt->execute([':payment_id' => $data['razorpay_payment_id']]);
$age_category = $stmt->fetchColumn();
            ...
            $skattor_info['level_category']=$age_category;    
                
            
            

            echo json_encode(["status" => "success", "message" => "Your Payment has Been Successfully Completed.", "payment_id" => $payment_id,"data" => $skattor_info]);
        } else {
