...
$columns = ['id', 'branch_id', 'user_id', 'username', 'password', 'status', 'created_at', 'updated_at', 'created_by', 'updated_by'];
    ...
    // Fetch data query
    $sqlData = "SELECT * FROM tbl_user  $whereClause ORDER BY $orderBy $orderDir";
    ...
    $sqlTotal = "SELECT COUNT(*) as total FROM tbl_user";
    $totalRecords = $pdo->query($sqlTotal)->fetchColumn();

    // Total filtered records count (with filtering)
    $sqlFiltered = "SELECT COUNT(*) as total FROM tbl_user $whereClause";
