...
try {
    ...
    $checkStmt = $pdo->prepare("SELECT logo_path FROM tbl_clubs WHERE id = :id");
    ...
    $stmt = $pdo->prepare("DELETE FROM tbl_clubs WHERE id = :id");
    if ($stmt->execute([':id' => $id])) {
        ...
        echo json_encode(["success" => true, "message" => "Club deleted successfully, logo removed"]);
    } else {
