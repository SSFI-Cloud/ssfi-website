...
try {
    ...
    // Base SQL query
    $whereClauses = ['1=1 and role_id=2'];
    $whereSql = "where 1=1 AND role_id = 2";
    ...
    if (!empty($state_id)) {
        ...
        $whereSql .= " AND u.state_id = " . intval($state_id);
    }
    if (!empty($district_id)) {
        $whereClauses[] = "u.district_id = :district_id";
        $bindings[':district_id'] = $district_id;
         $whereSql .= " AND u.district_id = " . intval($district_id);
    }
    
    if ($role_id==3 && $session_state_id) { // state login access
        $whereClauses[] = "u.state_id = :state_id";
        $bindings[':state_id'] = $session_state_id;
        $whereSql .= " AND u.state_id = " . intval($session_state_id);
    }
    if ($role_id==2 && $session_district_id) { //District Login Access
        $whereClauses[] = "u.district_id = :district_id";
        $bindings[':district_id'] = $session_district_id;
         $whereSql .= " AND u.district_id = " . intval($session_district_id);
    }
    ...
    if ($verify_id!='' && $verify_id!=null) {
        ...
        $whereSql .= " AND u.verified = " . intval($verify_id);
    }
    ...
    if (!empty($searchValue)) {
        $whereClauses[] = "(u.full_name LIKE :search OR u.mobile_number LIKE :search 
                            OR u.aadhar_number LIKE :search OR u.email_address LIKE :search )";
    ...
    // Total records count
    $sqlTotal = "SELECT COUNT(*) as total FROM tbl_user u $whereSql";
