...
try {
    $stmt = $pdo->prepare("INSERT INTO tbl_user 
        (role_id,full_name, mobile_number, gender, aadhar_number, email_address, state_id, district_id, residential_address, identity_proof, profile_photo, created_at, updated_at) 
        VALUES (:role_id,:full_name, :mobile_number, :gender, :aadhar_number, :email_address, :state_id, :district_id, :residential_address, :identity_proof, :profile_photo, NOW(), NOW())");
...
// }
    ...
    if ($stmt->execute()) {
        echo json_encode(["status" => 'success', "message" => "District Secretary registered successfully and waiting for Apporval"]);
    } else {
        echo json_encode(["status" => 'error', 'message' => 'Error executing query']);
    }
} catch (PDOException $e) {
