...
try {
    ...
    $columns = ['id', 'event_level_type_id', 'state_id', 'district_id', 'event_name', 'event_image', 'event_date', 'reg_start_date', 'reg_end_date', 'event_description', 'event_fees', 'event_remarks', 'status', 'title_of_championship', 'association_name', 'reg_no', 'date', 'venue', 'secretory_sign', 'president_sign'];
    $orderBy = isset($columns[$orderColumnIndex]) ? $columns[$orderColumnIndex] : 'id'; // Fallback to 'id'

    // Base SQL query for fetching data
$whereClause = 'WHERE 1=1 and event_level_type_id= 3';
$bindings = [];
...
if (!empty($searchValue)) {
    ...
    if ($state_id) {
        $count_condition .= " and s1.state_id=$state_id";
    }
    if ($district_id) {
        $count_condition .= " and s1.district_id=$district_id";
    }
    ...
    (
        ...
        WHERE tbl_event_registration.event_id = e.id $count_condition
                ...
                FROM tbl_events e
                LEFT JOIN tbl_states s ON e.state_id = s.id
    ...
    $sqlTotal = "SELECT COUNT(*) as total FROM tbl_events";
    $totalRecords = $pdo->query($sqlTotal)->fetchColumn();

    // Total filtered records count (with filtering)
    $sqlFiltered = "SELECT COUNT(*) as total FROM tbl_events $whereClause";
