...
// Log the request
error_log("Events deletion request received");
...
try {
    ...
    $stmt = $pdo->prepare("DELETE FROM tbl_events WHERE id = :id");
    if ($stmt->execute([':id' => $id])) {
        ...
        echo json_encode(["success" => true, "message" => "Events deleted successfully, logo removed"]);
    } else {
        echo json_encode(["success" => false, 'message' => 'Failed to delete Events']);
    }
