...
try {
    ...
    $columns = ['id', 'club_name', 'registration_number', 'contact_person', 'mobile_number', 'email_address', 'district_id', 'state_id', 'club_address', 'established_year', 'logo_path', 'status', 'created_at', 'updated_at'];
...
if (!empty($searchValue)) {
    ...
    // Fetch data query
    $sqlData = "SELECT c.*, s.state_name, d.district_name 
                FROM tbl_clubs c
    ...
    $sqlTotal = "SELECT COUNT(*) as total FROM tbl_clubs";
    $totalRecords = $pdo->query($sqlTotal)->fetchColumn();

    // Total filtered records count (with filtering)
    $sqlFiltered = "SELECT COUNT(*) as total FROM tbl_clubs $whereClause";
