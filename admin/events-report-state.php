            ...
            columns: [
                { data: null, render: (data, type, row, meta) => meta.row + 1 }, // Auto-increment row index
                {
                    ...
                    render: function (data) {
                        ...
                        // Show Edit Suit Size ONLY for event_id = 14
                        if (parseInt(data.event_id) === 14) {
                            buttons += `
                                <button class="btn btn-primary btn-xs"
                                    onclick="openEditSuitModal(${data.id}, ${data.event_id}, '${data.suit_size || ''}')">
                                    <i class="ri-edit-2-fill"></i> Edit Suit Size
                                </button>
                            `;
                        }
                
                        // Delete button (always visible if permission exists)
                        buttons += `
                            <button class="btn btn-danger btn-xs"
                                onclick="deleteSkaters(${data.id}, ${data.event_id})">
