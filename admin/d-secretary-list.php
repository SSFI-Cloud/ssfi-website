...
<div class="container-xxl flex-grow-1" style="padding: 0px 10px;">
    <div class="card mb-2">
        <div class="row p-2">
            <div class="col-md-8">
                <h6 class="card-header"><i class="ri-building-line"></i> District Secretary Details</h6>
            </div>
            <?php if(get_permission('d-secretary', 'is_add')){ ?>
        ...
        <div class="table-responsive text-nowrap">
            <table id="clubTable" class="table" style="width: 100%;"> 
                <thead>
                   <tr>
                        <th>Approved</th>
                        <?php if(get_permission('d-secretary', 'is_edit') || get_permission('d-secretary', 'is_view')|| get_permission('d-secretary', 'is_delete')){ ?>
                        <th>Actions</th>
                        <?php } ?>
                        <th>S.No</th>
                         <th>Full Name</th>
...
//   <button class="btn btn-danger btn-xs" onclick="deleteSecretary(${data.id})"><i class="ri-chat-delete-fill"></i> Delete</button>
    ...
    $table.DataTable({
        ...
        destroy: true,
        ajax: {
            url: `api/d-secretary/read.php${queryString}`,
            type: 'GET'
        },
        columns: [
              { 
            ...
            <?php if(get_permission('d-secretary', 'is_edit') || get_permission('d-secretary', 'is_view')|| get_permission('d-secretary', 'is_delete')){ ?>
            {
                ...
                render: (data) => `<?php if(get_permission('d-secretary', 'is_edit')){ ?>
                            <button class="btn btn-warning btn-xs" onclick="editSecretary(${data.id})"><i class="ri-edit-box-fill"></i> Edit</button>
                            <?php }  if(get_permission('d-secretary', 'is_delete')){ ?>
            ...
            // { 
            //     data:null,
            //     render: (data, type, row) => `<a href="view-d-secretary-info.php?id=${data.id}" class="">${data.full_name}</a>` 
            // },
            {
                data: null,
                render: (data, type, row) => {
                    let color = data.verified == 1 ? 'green' : 'red';
                    return `<a href="view-d-secretary-info.php?id=${data.id}" style="color: ${color};">${data.full_name}</a>`;
                }
            },
            // { data: 'full_name' },
            { data: 'gender' },
...
$('.add_btn').on('click', function() {
    window.location.href = 'd-secretary.php';
});
...
function editSecretary(id) {
   window.location.href = 'd-secretary.php?id='+id;
}
function viewSecretary(id) {
        window.location.href='view-d-secretary-info.php?id='+id;
...
$('#confirmDelete').click(function () {
    if (deleteId) {
        $.ajax({
            url: `api/d-secretary/delete.php`,
            type: 'POST',
