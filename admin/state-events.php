     ...
     <div class="col-sm-4">
         ...
         <div class="row mb-4">
          ...
          <div class="row mb-4">
            ...
            <div class="col-sm-8">
                <textarea id="event_remarks" name="event_remarks" class="form-control" required></textarea>
            </div>
        </div>
       </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function editClient(id) {
        ...
        $.get(`api/s-events/read_single.php?id=${encodeURIComponent(id)}`, function (data) {
    if (data) {
        getDropDown('tbl_states', 'state_id', 'state_name', {}, data.state_id);
        getDropDown('tbl_districts', 'district_id', 'district_name', { 'state_id': data.state_id }, data.district_id);
...
function submitForm() {
    ...
    let submitButton = $("#submitBtn"); // Ensure the submit button has an ID
    let formData = new FormData(form[0]); // Use FormData for file uploads
    let url = selectedId ? 'api/s-events/update.php' : 'api/s-events/create.php';
