...
if ($_SESSION['ssfi_event']['mobile_no'] === $mobile_no && $_SESSION['ssfi_event']['otp'] == $otp) {
    ...
    try {
        ...
        left join tbl_districts d on d.id=st.district_id WHERE st.membership_id = ?");
        ...
        if ($results) {
            ...
            if($results['district_id']!=$e_results['district_id']){
                echo json_encode(["status" => "error", "message" => "Your Trying to Register in Another District, Which is Not Allowed...","event_status" =>"0"]);
                exit;
            }
