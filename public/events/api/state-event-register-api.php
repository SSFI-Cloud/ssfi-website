...
$online_payment_id = $_POST['online_payment_id']!="" ? $_POST['online_payment_id'] : 0;
// $state_event_level_type_id = $_POST['state_event_level_type_id']!="" ? $_POST['state_event_level_type_id'] : 0;
$state_event_level_type_id = isset($_POST['state_event_level_type_id']) && $_POST['state_event_level_type_id'] != "" 
...
// error_log($online_payment_id.'sdfffdsfsdfsdf');
...
if($_results){
    ...
    if($_results[0]['payment_id']==""){ $ss="success"; } 
        ...
        echo json_encode(["status" => $ss,"data"=>$results, "message" => "Already Event Registration Completed, Kindly make the Payment", "membership_id" => $skattor_info['membership_id'],"order_id"=>$_results[0]['order_id'],"amount"=>$fees,'razorpay_api_secret'=>$Razorpay_api_secret]);
        exit;
}
...
//             $sq[]="('".$skater_id."','".$event_level_type_id."','".$event_id."','".$eligible_event_level_id[$i]."','".$e_results['session_id']."',$online_payment_id,$online_payment_id)";
//         }
//         $s.= implode(',', $sq);
//         //error_log($s);
...
//         $result=$statement->execute();
            ...
            if (!empty($state_event_level_type_id)) {
            $s = 'INSERT INTO `tbl_event_registration` 
                    (`skater_id`, `event_level_type_id`, `event_id`, `eligible_event_level_id`, `session_id`, `order_id`, `payment_id`) 
                    VALUES ';
        
            $values = [];
            $params = [];
        
            foreach ($state_event_level_type_id as $eligible_id) {
                $values[] = "(?, ?, ?, ?, ?, ?, ?)";
        
                $params[] = $skater_id;
                $params[] = $event_level_type_id;
                $params[] = $event_id;
                $params[] = trim($eligible_id); // remove any whitespace
                $params[] = $e_results['session_id'];
                $params[] = $online_payment_id;
                $params[] = $online_payment_id;
            }
...
$_results = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        $pdo->commit();
        echo json_encode(["status" => 'success',"data"=>$results, "message" => "To Complete Event Registration, Kindly Wait for Payment Verification", "membership_id" => $skattor_info['membership_id'],"order_id"=>$order_id,"amount"=>$fees,'razorpay_api_secret'=>$Razorpay_api_secret]);
