...
try{
  $pdo->beginTransaction();  
        $s='INSERT INTO `tbl_event_registration`(`skater_id`,`event_level_type_id`,`event_id`,`eligible_event_level_id`,session_id,order_id) VALUES ';
        for($i=0;$i<count($eligible_event_level_id);$i++){
            $sq[]="('".$skater_id."','".$event_level_type_id."','".$event_id."','".$eligible_event_level_id[$i]."','".$e_results['session_id']."','')";
        }
        $s.= implode(',', $sq);
        //error_log($s);
...
$_results = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        $pdo->commit();
        echo json_encode(["status" => 'success',"data"=>$results, "message" => "To Complete Event Registration, Kindly make the Payment to Hand", "membership_id" => $skattor_info['membership_id'],"order_id"=>$order_id,"amount"=>$fees,'razorpay_api_secret'=>$Razorpay_api_secret]);
