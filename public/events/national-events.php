...
$stmt = $pdo->prepare("SELECT * FROM `tbl_events` WHERE id=? AND NOW() BETWEEN reg_start_date AND reg_end_date");
$stmt->execute([$event_id]);
$e_results = $stmt->fetch(PDO::FETCH_ASSOC);

 if

// ($e_results && strtotime($e_results['reg_end_date']) >= time())
($e_results 
    && strtotime(date('Y-m-d')) >= strtotime('2025-12-01') 
    && strtotime($e_results['reg_end_date']) >= time())
            ...
            <div class="col-md-10">
                <div class="card shadow p-4 mt-4">
                    <div class="text-center mb-4">
                        <h5 class="text-primary font-weight-bold">Kindly Register Your Event Category Level</h5>
                    </div>

                    <form id="eventForm">
                        <div class="mb-4">
                            <h5>Hi <b class="confirm_name"></b>,</h5>
                            <p style="font-size: 16px;">
                                <strong>Membership ID:</strong> <span class="text-primary" style="color: #3e4095;"><b class="confirm_member_id"></b></span><br>
                                <strong>DOB / Gender:</strong> <span class="text-primary" style="color: #3e4095;"><b class="confirm_age_gender"></b></span><br>
                                <strong>Age:</strong> <span class="text-primary" style="color: #3e4095;"><b class="confirm_age" id="age_unique"></b></span><br>
                                <strong>Category:</strong> <span class="text-primary" style="color: #3e4095;"><b class="confirm_category"></b></span><br>
                                <strong>Category Age Group:</strong> <span class="text-primary" style="color: #3e4095;"><b class="confirm_level_category"></b></span><br>
                                <strong>Eligible Event Levels:</strong> <span class="text-primary" style="color: #3e4095;"><b class="event_levels"></b></span>
                            </p>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-4">
                                ...
                                <!--    <label for="online_payment_id" class="font-weight-bold">-->
                                <!--        Please enter a Skater Id -->
                                <!--        <span class="text-danger d-block small" style='color:red;'>-->
                                <!--            (Please make Sure ₹1300 payment and Enter Skater ID in UPI description/Notes)-->
                                <!--        </span>-->
                                <!--    </label>-->
                                <!--    <input type="text" id="online_payment_id" class="form-control" required />-->
                                <!--</div>-->

                                <!--<div class="text-center my-3">-->
                                <!--    <img src="image/qr-code-state.png" alt="Payment QR" class="img-fluid" style="max-width: 250px;" />-->
                                <!--</div>-->

                               

                                <!-- Hidden Fields -->
                                <input type="hidden" id="_session_id" required />
                                <input type="hidden" id="_skater_id" required />
                                <input type="hidden" id="_event_id" required />
                                <input type="hidden" id="_event_level_type_id" required />
                                <input type="hidden" id="state_event_level_type_id" required />
                            </div>

                            <div class="col-md-6" id="event_selection_div">
                                <!-- Event selection content loads here -->
                            </div>
                            <div class="col-md-6">
                                <label for="sname">Select SKIN SUITS *:<br></label>
                                <select name="suit_size" id="suit_size" required>
                        ...
                        <div class="text-center mt-4">
                            ...
                            <!--    Submit >>>-->
                            <!--</button>-->
                            <button style="color:white;" class="ui-btn ui-corner-all ui-shadow clr-btn-green ui-btn-icon-left ui-icon-pen clr-white btn-submit" type="submit">
                                           Submit>>>
                                        </button>
                        </div>
                    </form>
...
function pay(skater_id,event_id) {
    $.ajax({
        url: '../admin/api/front-api/get-national-event-payment.php',
        type: 'POST',
        dataType: 'json',
    ...
    $("#otpForm").submit(function (e) {
            ...
            $.ajax({
                url: "api/national_verify_otp.php",  // Replace with your actual API endpoint
                type: "POST",
                ...
                success: function(response) {
                    ...
                    if(res.status === "success") {
                        ...
                        //console.log(res.data.full_name);
                            ...
                            $('.event_levels').text(res.data.event_levels);
                            $('#state_event_level_type_id').val(res.data.event_level_type_ids);
                            $("#event_register").show();
                            ...
                            // $('#confirm_certicate_download').attr('href', "state-event-confirmation-pdf.php?id=" + res.data.id+"&event_id="+res.data.event_id);
                            ...
                            $('.confirm_level_category').text(res.data.level_category);
                            $('#confirm_profile').attr('src', "../admin/" + res.data.profile_photo);
                            $('#confirm_certicate_download').attr('href', "national-event-confirmation-pdf.php?id=" + res.data.id+"&event_id="+res.data.event_id);
                            $('.levels').html(res.data.levels);
                            $('#confirm_certicate_download').show();
                            $('#contiue_with_payment').hide();
                            $("#event_register").hide();
    ...
    $("#eventForm").submit(function (e) {
      ...
      $("#loader").show();  
        ...
        if(1){
            ...
            // let online_payment_id = $('#online_payment_id').val().trim();
            let state_event_level_type_id = $('#state_event_level_type_id').val().trim();
            let eligible_event_level_id = searchIDs; 
            let suit_size = $('#suit_size').val().trim();
            
            $.ajax({
                url: "api/national-razorpay.php",  // Replace with your actual API endpoint
                type: "POST",
                // data: {  state_event_level_type_id:state_event_level_type_id,skater_id: skater_id,event_level_type_id:event_level_type_id,event_id:event_id,eligible_event_level_id:eligible_event_level_id,online_payment_id:online_payment_id },
                data: {  state_event_level_type_id:state_event_level_type_id,skater_id: skater_id,event_level_type_id:event_level_type_id,event_id:event_id,eligible_event_level_id:eligible_event_level_id,suit_size:suit_size },
                dataType: "json",
                success: function(response) {
                    //console.log(response)
                    let res = response;
...
function openPaymentGateWay(order_id, amount, razorpay_api_key) {
    ...
    var options = {
        ...
        "handler": function (response) { 
            fetch("../admin/api/front-api/national-event-payment-capture.php", {
                method: "POST",
                headers: {
            ...
            .then(data => {
                ...
                $('#confirm_certicate_download').attr('href', "national-event-confirmation-pdf.php?id=" + data.data.id+"&event_id="+data.data.event_id);
                $('#contiue_with_payment').hide();
                $("#event_register").hide();
                $("#event_confirmation").show();
            })
...
$(document).ready(function () {
    ...
    $('#verify_member_id').on('keyup', function () {
        var input = $(this).val();
        validSelection = false; // Reset validity on each keyup
        $('#mobBtn').prop('disabled', true); // Always disable first

        if (input.length >= 4) {
            $.ajax({
                url: 'api/fetch_members_national.php',
                method: 'POST',
                data: { query: input },
    ...
    $(document).on('click', '.suggestion-item', function () {
        ...
        $('#member_suggestion').fadeOut();
        validSelection = true;
        $('#mobBtn').prop('disabled', false); // Enable button on valid selection
    });
    ...
    $('form').on('submit', function (e) {
        if (!validSelection) {
            e.preventDefault();
            alert('Please select a valid member ID from the suggestions.');
        }
    });
});
